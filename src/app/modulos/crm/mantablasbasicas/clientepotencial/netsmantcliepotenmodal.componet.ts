    ...
    template: `
        <kendo-dialog title="{{ptitulo}}" 
          ...
          <div class="alert alert-danger" *ngIf="enlistaerror">
            ...
            <crm-addcliepoten *ngIf="cargoConfig && ptipomant=='A'" 
            vparcaptura="{{pvaparam}}"></crm-addcliepoten>
            <crm-editcliepoten *ngIf="cargoConfig && ptipomant=='E'" 
                vparcaptura="{{pvaparam}}" 
                vid="{{pid}}" >
             </crm-editcliepoten>
             <crm-vertcliepoten *ngIf="cargoConfig && ptipomant=='C'"  
                 vparcaptura="{{pvaparam}}" 
                 vid="{{pid}}" >
            </crm-vertcliepoten>
            </kendo-dialog>
...
export class Netsmantcliepotenmodal implements OnInit {
    ...
    inicializacpaturatabla(objeto) {
        ...
        this.service.getNetsolinObjmantbasica(objeto)
            .subscribe(result => {
                ...
                //Cambiar titulo 
                NetsolinApp.objtitmodulo.titulo = result0.title;
