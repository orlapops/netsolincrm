...
// export interface Item { codigo: string; nombre: string; }

@Component({
  selector: 'tienda-addcategoria',
  templateUrl: './adicionar.component.html',
  styleUrls: ['./adicionar.component.css']
})
export class AddregCategoriaComponent implements OnInit {
  ...
  constructor(private mantbasicaService: MantbasicaService,
    ...
    private fbDb: AngularFirestore
      ...
      this.items = fbDb.collection('categorias').valueChanges();      
  }
  ngOnInit() {
  ...
  inicializaForm() {
    ...
    avalida.push(Validators.required);
     this.libmantab.defineValidaCampo(this.tablaForm, "cod_categor", avalida);
     this.libmantab.defineValidaCampo(this.tablaForm, "nombre", avalida);
  ...
  */
  ...
  onSubmit() {
    ...
    this.regTabla.id_categoria=999;    
    ...
    var fcod_categor = this.regTabla.cod_categor;
    console.log('item a adicionar ennetsolin fcod_categor: '+fcod_categor);
    // var fnombre = this.regTabla.nombre;
    // console.log('item a adicionar ennetsolin fnombre:'+fnombre);
    // const itemb: Item ={fcod_categor,fnombre};
    // itemb.codigo = fcod_categor;
    // itemb.nombre = fnombre;
    // console.log('item a adicionar ennetsolin temb');
    // console.log(itemb);
  
    // itema : Item = {fcod_categor, fnombre};
    console.log('item a adicionar ennetsolin');
    ...
    this.mantbasicaService.postregTabla(this.regTabla, this.ptablab, this.paplica,this.pcampollave, this.pclase_nbs, this.pclase_val, this.pcamponombre)
      .subscribe(newpro => {
        console.log('item adiciono en netsolin');
        this.grabando = false;
        var result0 = newpro[0];
      
          //adicionar en firebase
          console.log('item a adicionar en firebase');
          // console.log(itemb);
          // this.itemsCollection.add(itemb);
          this.fbDb.collection('categorias').doc(fcod_categor).set(this.regTabla)
          .then(function() {
            console.log("Se adiciono en firebase!");
        })
        .catch(function(error) {
            console.error("Error writing documento en fiebase: ", error);
        });
        
          // this.itemsCollection.doc("categorias").set(itemb);
          console.log('item adicionado en firebase');

          this.grabo = true;
          this.tablaForm.reset();

          this.showMensaje('Se adiciono categoria.');
