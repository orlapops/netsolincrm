...
@Component({
  selector: 'tienda-addsubcatego',
  templateUrl: './adicionar.component.html',
  styleUrls: ['./adicionar.component.css']
})
export class AddregSubcategoComponent implements OnInit {
  ...
  inicializaForm() {
    ...
    avalida.push(Validators.required);
     ...
     this.libmantab.defineValidaCampo(this.tablaForm, "id_categoria", avalida);
     this.libmantab.defineValidaCampo(this.tablaForm, "icono", avalida);
     this.libmantab.defineValidaCampo(this.tablaForm, "cod_subcatg", avalida);
  ...
  onSubmit() {
    ...
    this.regTabla.id_subcatego= 999;    
    ...
    this.mantbasicaService.postregTabla(this.regTabla, this.ptablab, this.paplica,this.pcampollave, this.pclase_nbs, this.pclase_val, this.pcamponombre)
      .subscribe(newpro => {
        ...
        var result0 = newpro[0];
          ...
          this.showMensaje('Se adiciono Subcategoria.');
  ...
  verComboid_categoria(event, pcamporecibe, pcamporetorna) {
   ...
   // this.cargorefere = false;
    ...
    if (lvalor) {
      this.mantbasicaService.getregTabla(lvalor, "TIEN_CATEGOR", "31", "id_categoria", "", "", "nombre")
        .subscribe(regTabla => {
          if (typeof regTabla.isCallbackError == "undefined") {
            ...
            this.libmantab.asignaValorcampoform(this.tablaForm,"id_categoria","");
          }
        });
    } else {
      //ad producto
